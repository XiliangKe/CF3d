...
def farthest_point_sample(xyz, npoint):
    """
    最远点采样，从一个点云中采样出所需点的点数，并且点与点之间的距离要足够远
    Input:
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    """
    寻找球形领域中的点
    query_ball_point函数用于寻找球形领域中的点。输入中radius为球形领域的半径，nsample为每个领域中要采样的点，new_xyz为S个球形领域的中心（由最远点采样在前面得出），xyz为所有的点云；
    Input:
...
def sample_and_group(npoint, nsample, xyz, points, density_scale = None):
    """
    将整个点云分散成局部的group，对每个group都可以用到pointnet单独的提取局部的全局特征
    Input:
        npoint:
        nsample:
    ...
    #xyz: 所有点云
    #new_xyz：每个球形领域的中心
    #idx：利用query_ball_point和index_points将原始点云通过new_xyz 作为中心分为npoint个球形区域其中每个区域有nsample个采样点
    #grouped_xyz:每个球形区域的所有点
...
def sample_and_group_all(xyz, points, density_scale = None):
    ...
    euclidean = torch.sqrt((grouped_xyz ** 2).sum(dim=-1, keepdim=True))
    if points is not None:
...
class PointConvDensitySetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        grouped_xyz = grouped_xyz_norm.permute(0, 3, 2, 1)
        weights = self.weightnet(grouped_xyz)     
        new_points = new_points * grouped_density.permute(0, 3, 2, 1)  #区域中的点特征乘以密度
        new_points = torch.matmul(input=new_points.permute(0, 3, 1, 2), other = weights.permute(0, 3, 2, 1)).view(B, self.npoint, -1)
        new_points = self.linear(new_points)
        new_points = self.bn_linear(new_points.permute(0, 2, 1))
        new_points = F.relu(new_points)
