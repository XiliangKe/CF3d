...
from utils.pointconv import PointConvDensityClsSsg
...
class  Model:
    ...
    def train(self):
        print('Now we use Scannet as our traindata')
        ...
        trainDataLoader = DataLoader(ScanObjectDataLoader(self.opts, partition='training'),
                                  batch_size=self.opts.batch_size, shuffle=True, drop_last=False)
        testDataLoader = DataLoader(ScanObjectDataLoader(self.opts,partition='test'),
        ...
        # TRAIN_DATASET = PartNormalDataset(root=root, npoints=self.opts.npoint, split='train', normal_channel=self.opts.normal)
        # trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=self.opts.batch_size, shuffle=True, num_workers=10, drop_last=True)
        # TEST_DATASET = PartNormalDataset(root=root, npoints=self.opts.npoint, split='test', normal_channel=self.opts.normal)
        # testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=self.opts.batch_size, shuffle=False, num_workers=10)
        # self.log_string("The number of training data is: %d" % len(TRAIN_DATASET))
        # self.log_string("The number of test data is: %d" % len(TEST_DATASET))
        ...
        self.dim = 0
        classifier = PointConvDensityClsSsg(num_class)
        ...
        for epoch in range(start_epoch, self.opts.epoch):
            ...
            for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
                points, target = data #[24, 1024, 3], [24, 1]
                target = target[:, 0] #[24]
                points, target = points.cuda(), target.cuda().long() #[24, 1024, 3], [24]

                points = PointcloudScaleAndTranslate(points) #[24, 1024, 3]

                points = points.transpose(2, 1).contiguous() #[24, 3, 1024]
                noise = 0.02 * torch.randn(self.opts.batch_size, 1024).cuda() #[24, 1024]
                classifier = classifier.train()
                augmentor = augmentor.train()
                optimizer_a.zero_grad()
                aug_pc = augmentor(points, noise)

                center_cls, pred_pc, pc_tran, pc_feat = classifier(points) #[12, 40], [12, 40], none, [12, 1024]
                center_aug, pred_aug, aug_tran, aug_feat = classifier(aug_pc) #[12, 40], [12, 40], none, [12, 1024]
                ...
                crossEntropyLoss = loss_utils.cls_loss(pred_pc, pred_aug, target, pc_tran, aug_tran, pc_feat, aug_feat, ispn=ispn)
                centerLossPc = loss_utils.center_loss(center_cls, target, 0.1)
                clsLoss = crossEntropyLoss + centerLossPc
            ...
            if (test_acc >= best_tst_accuracy) and test_acc >= 0.70:# or (epoch % self.opts.epoch_per_save == 0):
